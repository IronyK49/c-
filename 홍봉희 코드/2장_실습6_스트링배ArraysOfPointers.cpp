...
*/
void bubbleSort(const char* arr[ROWS], int n);
void showData(const char* message, const char* data[ROWS], int n);
int main() {
    //스트링 배열 초기화 방법 2가지 구분
    //char strings[ROWS][SIZE] = { "apple", "banana", "orange", "grape", "kiwi" };//실습5에서 연습
    const char  *strings[ROWS] = { "apple", "banana", "orange", "grape", "kiwi" };//포인터가 스트링을 가리킬 때 주의할 점
...
void showData(const char* message, char* data[ROWS], int n) {
    printf("%s\n", message);
    for (int i = 0; i < n; i++) {
        printf("%s\n", *(data + i));
    }
