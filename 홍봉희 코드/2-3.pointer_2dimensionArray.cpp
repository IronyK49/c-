...
void testPointers(int a[][4](중요 포인터가 가르키는 배열의 크기가 필요 a[]=(*a) =/= *a[4]), int* b, int* c[], int(*d)[4]) {
	printf("*b : %d\n", *b);
	printf("b[0] : %d\n", b[0]);
	printf("*(++b) : %d\n", *(++b));
	printf("*(++b) : %d\n\n\n", *(++b));
	...
	/*
	d[0] = a[0]; // 왼쪽 피연산자인 d[0]이 lvalue가 아니므로 error 발생 (d[0] = *d d[0]은 배열 d를 가리킴)
	d[1] = a[1]; // 왼쪽 피연산자인 d[0]이 lvalue가 아니므로 error 발생  (d[1] = d+1) 
