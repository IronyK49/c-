...
//acces specifier 접근 명시자  , upcasting 제한됨
...
public:
    ...
    Animal* getAnimal() { return this; } // Animal 포인터 노출 // 클래스에선 가능 main에선 상속관계가 숨겨져 있기 때문에 안됨 
};
...
public:
    ...
    //Animal* getAnimal() { return this; }//클래스 내에서는 upcasting 허용 static_cast dynamic_cast 는 중요함! 
...
int main() {
    ...
    /*
    Cow c(...);
    Animal* a = &c;  // ❌ 오류! private 상속이므로 외부에서 업캐스팅 금지 animal* a = c1 : upcasitng
    //upcasting 숨긴다는 것은 상속관계도 숨겨버림 public 상속일때만 가능 
    */
